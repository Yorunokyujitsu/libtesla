...
namespace tsl {
	...
	namespace gfx {
			...
			std::pair<u32, u32> drawString(const char* string, bool monospace, u32 x, u32 y, float fontSize, Color color) {
                ...
                do {
                    ...
                    if (glyph->glyphBmp != nullptr && !std::iswspace(currCharacter) && fontSize > 0 && color.a != 0x0)
					    this->drawGlyph(currCharacter, currX + glyph->bounds[0], currY + glyph->bounds[1], color, glyph->currFont, glyph->currFontSize);
		...
		private:
             ...
             */
            std::string m_MainFrameButtonText{"\uE0E1  뒤로     \uE0E0  확인"};
			...
			Result initFonts() {
				...
				if(R_SUCCEEDED(setInitialize())) {
                    ...
                    if (R_SUCCEEDED(setGetSystemLanguage(&languageCode))) {
                        SetLanguage setLanguage{SetLanguage_KO};
	...
	namespace elm {
		...
		public:
			...
			virtual void draw(gfx::Renderer *renderer) override {
				...
				if (!deactivateOriginalFooter) renderer->drawString("\uE0E1  뒤로     \uE0E0  확인", false, 30, 693, 23, a(defaultTextColor));
